...
import com.example.email_Scheduler.quartzJob.EmailJob;
...
public class EmailSchedulerController {
    ...
    public ResponseEntity<EmailResponse> scheduleEmail(@Valid @RequestBody EmailRequest emailRequest){
        try{
            ...
            if(dateTime.isBefore(ZonedDateTime.now())){
                EmailResponse emailResponse =new EmailResponse(false,"dataTime must be after current time.");
    ...
    private JobDetail buildJobDetail(EmailRequest scheduleEmailRequest)
    {
         ...
         return JobBuilder.newJob(EmailJob.class)
                 .withIdentity(UUID.randomUUID().toString(), "email_job")
